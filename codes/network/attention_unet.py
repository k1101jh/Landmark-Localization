...
from network.blocks import AttentionBlock
...
class AttentionUNet(nn.Module):
    def __init__(self, in_channels, out_channels, channels):
        ...
        for channel_idx in range(len(channels) - 1, 0, -1):
            ...
            self.up_layers.append(
                AttentionBlock(
                    F_g=channels[channel_idx - 1],
                    F_l=channels[channel_idx - 1],
                    F_int=channels[channel_idx - 1] // 2,
                )
            )
    ...
    def forward(self, x):
        ...
        for layer_idx in range(len(self.channels) - 1):
            x = self.up_layers[layer_idx * 3](x)
            x = self.up_layers[layer_idx * 3 + 1](x, s[layer_idx])
            x = torch.cat((x, s[layer_idx]), dim=1)
            x = self.up_layers[layer_idx * 3 + 2](x)
